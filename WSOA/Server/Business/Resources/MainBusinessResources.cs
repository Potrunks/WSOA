namespace WSOA.Server.Business.Resources
{
    public static class MainBusinessResources
    {
        public const string TECHNICAL_ERROR = "Erreur technique. Contactez l'administrateur";
        public const string NULL_OBJ_NOT_ALLOWED = "L'objet {0} ne peut pas être null dans la méthode {1}";
        public const string NULL_OR_EMPTY_OBJ_NOT_ALLOWED = "L'objet {0} ne peut pas être null ou vide dans la méthode {1}";
        public const string USER_NO_CONNECTED = "Vous êtes deconnecté";
        public const string USER_CANNOT_PERFORM_ACTION = "Action non permise";
    }
}
